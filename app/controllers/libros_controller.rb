class LibrosController < ApplicationController
  ...
  def index
    if params[:query].present?
      @libros = Libro.where("autor LIKE ? OR nombre LIKE ? OR genero LIKE ?", "%#{params[:query]}%", "%#{params[:query]}%", "%#{params[:query]}%")
    else
      @libros = Libro.all
    end
  end
  ...
  def destroy
    ...
    respond_to do |format|
      format.html { redirect_to libros_path, status: :see_other, notice: "Libro was successfully destroyed." }
  ...
  private
    ...
    def libro_params
      params.require(:libro).permit(:autor, :nombre, :genero)
    end
end
